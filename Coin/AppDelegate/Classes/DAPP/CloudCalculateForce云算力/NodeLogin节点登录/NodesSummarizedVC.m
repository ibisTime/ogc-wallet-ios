...
@interface NodesSummarizedVC ()<UIScrollViewDelegate>
...
@implementation NodesSummarizedVC
...
- (void)viewDidLoad {
    ...
    UILabel *priceNameLbl = [UILabel labelWithFrame:CGRectMake(0, 30, (SCREEN_WIDTH - 30)/2 - 2, 40) textAligment:(NSTextAlignmentRight) backgroundColor:kClearColor font:FONT(12) textColor:nil];
    [priceNameLbl theme_setTextColorIdentifier:GaryLabelColor moduleName:ColorName];
    priceNameLbl.text = @"锁仓金额";
    [backView addSubview:priceNameLbl];
    
    
    UILabel *allWinNode = [[UILabel alloc]initWithFrame:CGRectMake(priceNameLbl.xx + 4, 40, 54, 20)];
    allWinNode.text = @"共盈节点";
    allWinNode.textColor = kWhiteColor;
    allWinNode.font = FONT(11);
    kViewRadius(allWinNode, 2);
    allWinNode.textAlignment = NSTextAlignmentCenter;
    allWinNode.backgroundColor =  kHexColor(@"#F4AC71");
    [backView addSubview:allWinNode];
    ...
    self.WeiGreLabel = [[UILabel alloc]initWithFrame:CGRectMake(SCREEN_WIDTH/kPageCount/2 - 12, 216 , 24, 3)];
...
- (void)setupScrollView{
    ...
    self.scroll = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 220 , SCREEN_WIDTH, SCREEN_HEIGHT)];
...
- (void)setupSelectBtn{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        _WeiGreLabel.frame = CGRectMake((self.currentPages + 1)*SCREEN_WIDTH/kPageCount - SCREEN_WIDTH/kPageCount/2 - 15, 216, 30, 3);
    }];
}
