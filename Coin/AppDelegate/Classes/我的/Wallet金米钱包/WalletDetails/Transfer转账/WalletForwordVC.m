...
@property (nonatomic , strong)CustomFMDBModel *walletModel;
...
//    self.title = [LangSwitcher switchLang:@"转账" key:nil];
    self.titleText.text = [LangSwitcher switchLang:@"转账" key:nil];
...
//    [self loadPwd];
    //矿工费
    _walletModel = [CustomFMDBModel mj_objectWithKeyValues:[CustomFMDB FMDBqueryUseridMnemonicsPwdWalletName]];
...
- (void)getgamProce
{
    ...
    CoinModel *coin = [CoinUtil getCoinModel:self.currency.symbol];
    if ([coin.type isEqualToString:@"0"] || [coin.type isEqualToString:@"0T"])
    {
        TLNetworking *net =[TLNetworking new];
        net.showView = self.view;
        net.code = @"802109";
    ...
    else if ([coin.type isEqualToString:@"1"])
    {
        [self loadUtxoList];
    }
    
    else if ([coin.type isEqualToString:@"3"])
    {
        [self loadUtxoList];
    }
    else if ([coin.type isEqualToString:@"2"] || [coin.type isEqualToString:@"2T"])
    {
...
//        UISlider * slider = sender;
        ...
        if ([self.currency.type isEqualToString:@"0"] || [self.currency.type isEqualToString:@"0T"]) {
...
-(void)transfer
{
    ...
    if ([self.currency.type isEqualToString:@"0"]) {
        if ([self.currency.symbol isEqualToString:@"BTC"] || [self.currency.symbol isEqualToString:@"USDT"]) {
            ...
            if ([self.balanceTF.text isEqualToString:self.currency.symbol]) {
...
//                     eth
                     ...
                     [net postWithSuccess:^(id responseObject) {
                         ...
                         if ([responseObject[@"data"][@"isSuccess"] integerValue] == 1) {
                             _resultStr = [MnemonicUtil sendTransactionWithMnemonicWallet:self.walletModel.mnemonics address:[self.balanceTF.text lowercaseString] amount:gaspic gaspic:[NSString stringWithFormat:@"%ld",(long)[self.pricr integerValue]] gasLimt:@"210000"];
                         }else
                         {
                             _resultStr = [MnemonicUtil sendTransactionWithMnemonicWallet:self.walletModel.mnemonics address:[self.balanceTF.text lowercaseString] amount:gaspic gaspic:[NSString stringWithFormat:@"%ld",(long)[self.pricr integerValue]] gasLimt:@"21000"];
                         }
                         [self RadioPrompt];
                     } failure:^(NSError *error) {
...
//                     btc转账
                     ...
                     return ;
                 }else if ([self.currency.type isEqualToString:@"3"])
                 {
//                     usdt转账
                     [self utxoTransfer];
                     return;
                 }
                 else if ([self.currency.type isEqualToString:@"2"])
                 {
//                     wan
                     TLNetworking *net = [TLNetworking new];
                     net.code = @"802108";
                     ...
                     [net postWithSuccess:^(id responseObject) {
                         ...
                         if ([responseObject[@"data"][@"isSuccess"] integerValue] == 1) {
                             _resultStr = [MnemonicUtil sendWanTransactionWithMnemonicWallet:self.walletModel.mnemonics address:[self.balanceTF.text lowercaseString] amount:gaspic gaspic:[NSString stringWithFormat:@"%ld",(long)[self.pricr integerValue]] gasLimt:@"210000"];
                         }else
                         {
                             _resultStr = [MnemonicUtil sendWanTransactionWithMnemonicWallet:self.walletModel.mnemonics address:[self.balanceTF.text lowercaseString] amount:gaspic gaspic:[NSString stringWithFormat:@"%ld",(long)[self.pricr integerValue]] gasLimt:@"21000"];
                         }
                         [self RadioPrompt];
                     } failure:^(NSError *error) {
                         //        [MBProgressHUD hideHUDForView:self.view animated:YES];
                 ...
                 {
                     CoinModel *coin = [CoinUtil getCoinModel:self.currency.symbol];
                     _resultStr = [MnemonicUtil sendEthTokenTransactionWithAddress:self.walletModel.mnemonics contractAddress:coin.contractAddress address:[self.balanceTF.text lowercaseString] amount:self.tranAmountTF.text gaspic:self.pricr gasLimt:@"210000"];
                     [self RadioPrompt];
                 }
             });
         }else{
...
- (void) testSpendCoins:(NSString *)to : (NSString*)count :(NSString*)free {
    NSArray *words = [self.walletModel.mnemonics componentsSeparatedByString:@" "];
