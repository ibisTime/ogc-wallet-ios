...
//        [self addSubview:goodBtn];
        ...
        lineView.frame = CGRectMake(19.5, 0, 1, contactLbl.yy + 8);
    }
    return self;
}
...
-(void)setModel:(AlertsModel *)model
{
    ...
    NSLog(@"%@",result);
    _nameLbl.frame = CGRectMake(30, 28.5 + 11.5, SCREEN_WIDTH - 45, 0);
//    _nameLbl.text = result;
    _nameLbl.attributedText = [UserModel ReturnsTheDistanceBetween:result];;
    _nameLbl.numberOfLines = 0;
    [_nameLbl sizeToFit];
    
    
    _contactLbl.frame = CGRectMake(30,_nameLbl.yy + 7, SCREEN_WIDTH - 45, 0);
    ...
    _contactLbl.attributedText = [UserModel ReturnsTheDistanceBetween:result1];
    _contactLbl.numberOfLines = 0;
    [_contactLbl sizeToFit];
    
    self.lineView.frame = CGRectMake(19.5, 0, 1, _contactLbl.yy + 8);
}
